...
대형 언어 모델 속 감정 개념을 다룬 연구들은, 모델 내부에서 "감정 벡터"를 찾아낼 수 있고
그것이 실제로 행동을 일으킨다는 점을 보여 왔다. Ludex는 이를 Core(뇌)에서 나온 발견으로
받아들인다. Ludex는 Shell(몸) 프로젝트이므로, Core의 감정 상태를 Shell이 읽고 대응하는
어댑터를 만든다.
