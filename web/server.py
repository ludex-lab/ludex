...
async def rag_query(req: RAGRequest):
    ...
    sources = []
    if memory:
        results = memory.handle_recall(req.message, memory_type="semantic", limit=3)
        sources = list(set(r.memory.source for r in results if r.memory.source))

    # Build context
    context = ""
    if results:
        parts = [f"[{r.memory.source}, rel:{r.relevance:.2f}] {r.memory.content}" for r in results]
        context = "\n---\n".join(parts)

    prompt = f"Context:\n{context}\n\nQuestion: {req.message}" if context else f"No documents found. Question: {req.message}"

    # Track user behavior
    if humoral:
        humoral.handle_report_interaction(
            opponent="user", opponent_action="COOPERATE",
            my_action="COOPERATE", my_score=3, opponent_score=3,
        )

    result = await asyncio.to_thread(engine.handle_submit, prompt)
...
async def rag_ingest(req: IngestRequest):
    """Add document text to RAG memory."""
    org = agents.get(req.session_id)
    if not org:
        return {"error": "Session not found"}
    ...
    memory = org.get_block("memory")
    if not memory:
        return {"error": "No memory block in this agent"}
    ...
    for i, chunk in enumerate(chunks):
        memory.handle_remember(
            content=chunk, memory_type="semantic",
            tags=["document", req.source], importance=0.7,
            source=req.source,
        )

    return {"chunks": len(chunks), "mem_count": memory.count}
...
@app.post("/api/rag/ingest-folder")
async def rag_ingest_folder(req: IngestFolderRequest):
    """Load all text files from a folder into RAG memory."""
    org = agents.get(req.session_id)
    if not org:
        return {"error": "Session not found"}
    ...
    memory = org.get_block("memory")
    if not memory:
        return {"error": "No memory block in this agent. Did you select RAG agent type?"}
    ...
    import glob
    extensions = [".txt", ".md", ".py", ".json", ".csv", ".html", ".yml", ".yaml", ".toml", ".cfg", ".ini", ".log"]
    files_loaded = []
    total_chunks = 0
        ...
        for filepath in glob.glob(os.path.join(folder, f"**/*{ext}"), recursive=True):
            try:
                ...
                for i, chunk in enumerate(chunks):
                    memory.handle_remember(
                        content=chunk, memory_type="semantic",
                        tags=["document", filename], importance=0.7,
                        source=filename,
                    )

                files_loaded.append(f"{filename} ({len(chunks)} chunks)")
                total_chunks += len(chunks)
            except Exception as e:
                files_loaded.append(f"{os.path.basename(filepath)} (error: {e})")

    return {
        "files_loaded": len([f for f in files_loaded if "error" not in f]),
        "total_chunks": total_chunks,
        "files": files_loaded[:20],  # limit display
